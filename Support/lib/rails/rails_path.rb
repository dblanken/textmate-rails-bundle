...
class RailsPath
  ...
  def controller_name
    ...
    when :lib_test then name.sub!(/_test$/, '')
    when :lib then name
    else
  ...
  def file_type
    ...
    @file_type =
      case @filepath
      when %r{/test/controllers/(.+\.(rb))$}             then :functional_test
      ...
      when %r{/test/lib/(.+\.(rb))$} then :lib_test
  ...
  def controller_name_possibles_modified_for(type)
    ...
    when :lib_test then controller_name + '_test'
  ...
  def stubs
    { :functional_test => 'test/controllers',
